...
В ходе работ исходный проект изменялся, код оптимизировался, добавлялись отчеты. Ход изменений можно посмотреть ко коммитам. Так же можно ориетироваться на текущий README. В нем записаны все необходимые ссылки на отчеты, код и изменения в коммитах.
...
На первом этапе в проекте broken-app необходимо было исправить
1. Починить тесты
2. Устранить утечки памяти
3. Устаринть ошибки работы с памятью
...
Результат анализа
https://raw.githubusercontent.com/NApofis/broken_app/a05f738968177d85ecdebae6c5c031d77b508edc/checks/step-2/broken-app/flame.svg
...
Показатели эталонного проекта можно посмотреть [тут](https://raw.githubusercontent.com/NApofis/broken_app/a05f738968177d85ecdebae6c5c031d77b508edc/checks/step-2/reference-app/flame.svg)
...
Предварительные результат можно посмотреть [тут](https://napofis.github.io/broken_app//checks/step-3/broken-app/before/criterion/report/).
Результаты показали нузкую производительность методов `slow_fib_broken` и `slow_dedup_broken`. Метод `sum_even_broken` отрпботал более менее нормально.
...
3. Изменен иетод [sum_even](https://github.com/NApofis/broken_app/commit/03f8c389e4f98640b4f5924eb2340f7b1e9bf348#diff-eb4943c9ee3ec2beac689bded1b0376c6e6c8ada07e827b042d6d95764b08072L7). Для его работы не требуется unsafe код, а достаточно только фильтрации и симирования через стандартные методы.
4. Изменен метод [normalize](https://github.com/NApofis/broken_app/commit/03f8c389e4f98640b4f5924eb2340f7b1e9bf348#diff-eb4943c9ee3ec2beac689bded1b0376c6e6c8ada07e827b042d6d95764b08072L48). Вместо двойного копирования строки, из за которой приходилось выделять память и сразу ее опустащать, был сделан один цинл for. Так было сделано предвыделение памяти что поможет если исходная строка большая. Прадва это же  изменение сыграет плохую роль если строка будет состоять только из пробелов, но этот маловероятный сценарий.
...
8. Итоговый `perf + flamegraph` [тут](https://raw.githubusercontent.com/NApofis/broken_app/refs/heads/main/checks/step-3/broken-app/after/flame.svg)
